...
def executar_importacao_segundo_plano(df, usr_dono, perfil_ativo, limite_permitido):
    ...
    try:
        ...
        # 1. LEITURA RÁPIDA E VALIDAÇÃO DOS DADOS EM MEMÓRIA (Limite de progresso até 20%)
            ...
            with conn.cursor() as cur:
                for index, row in df.iterrows():
                    ...
                    lista_dados_pai.append({
                        ...
                        "proxima": proxima_pesquisa,
                        "dias": dias,
                        "prod_dados": prod_dados,
                        "flag": flag_concorrente
                    })
                    
                    # Progresso máximo de 20% na fase de leitura
                ...
                for idx, item in enumerate(lista_dados_pai):
                    # Altera o status dinamicamente mostrando a contagem atualizada entrando no banco
                    status_importacao["status"] = f"Gravando agendamento {idx + 1} de {total_pais} no banco..."
                    
                    cur.execute(query_pai, (item["ean"], item["cidade"], item["proxima"], item["dias"], usr_dono))
                    ...
                    # Processa concorrência varrendo os índices
                    concorrentes_lista = []
                    if item["flag"] == "sim":
                    ...
                    # O cálculo agora vai de 20% até 95% proporcionalmente à gravação física das linhas
                    status_importacao["progresso"] = 20 + int(((idx + 1) / total_pais) * 75)
                
                # Executa o Bulk Insert final dos concorrentes
                if lista_concorrentes:
                    status_importacao["status"] = "Finalizando vínculos de concorrentes em lote..."
                    cur.executemany("INSERT INTO dados_concorrentes (dados_id, ean_concorrente) VALUES (%s, %s)", lista_concorrentes)
                
            conn.commit()
            
        # Finaliza cravando 100% após o commit seguro no Postgres
        status_importacao["progresso"] = 100
...
def index():
    ...
    if tela == "painel":
        ...
        try:
            ...
            with GerenciadorConexao() as conn:
                with conn.cursor() as cur:
                    # Mantemos o SQL original exatamente igual para NÃO quebrar os índices row[X] do seu loop
                    sql_base = """
                        SELECT d.id, d.codigo_barras, d.cidade, d.status, d.data_cadastro, d.dias, 
                               p.codigo_produto, p.descricao_item, d.usuario_dono 
                        FROM dados d
                        LEFT JOIN produtos p ON p.ean_principal = d.codigo_barras
                    """
                    
                    if perfil_ativo == 'ADMIN':
                        cur.execute(sql_base + " ORDER BY d.id DESC")
                    else:
                        cur.execute(
                            sql_base + " WHERE LOWER(d.usuario_dono) = %s ORDER BY d.id DESC", 
                    ...
                    for row in rows:
                        ...
                        cod_produto_real = row[6] if len(row) > 6 and row[6] else "-"
                        desc_produto_real = row[7] if len(row) > 7 and row[7] else "Produto Não Cadastrado"
                        # 🎯 CAPTURA O USUÁRIO DONO DO BANCO (Última coluna adicionada no SELECT)
                        usuario_dono_registro = str(row[8]).strip().lower() if len(row) > 8 and row[8] else ""

                        # Consulta os concorrentes da tarefa ativa
                        cur.execute("SELECT ean_concorrente FROM dados_concorrentes WHERE dados_id = %s", (id_real,))
                        rows_conc = cur.fetchall()
                        ...
                        if not concorrentes_lista and not possui_bloqueio and cod_produto_real != "-":
                            cur.execute("""
                                ...
                                FROM produtos 
                                WHERE LOWER(codigo_produto) = %s
                            """, (str(cod_produto_real).strip().lower(),))
                        ...
                        item_tarefa = [
                            id_real,          # t[0]
                            ean_real,         # t[1]
                            concorrentes_real,# t[2]
                            cidade_real,      # t[3]
                            status_real,      # t[4]
                            data_real,        # t[5]
                            dias_real,        # t[6]
                            cod_produto_real, # t[7]
                            desc_produto_real # t[8]
                        ]
                        
                        # 🧠 FILTRAGEM DINÂMICA INTEGRAÇÃO BACKEND
                        corresponde = True
                        ...
                        if status_filtro:
                            ...
                            if status_filtro == 'concluida' and not ('concluid' in status_limpo or 'processado' in status_limpo):
                                corresponde = False
                        ...
                        # 🎯 NOVO: Filtro por Operador/Usuário logado
        ...
        try:
            with GerenciadorConexao() as conn:
                with conn.cursor() as cur:
                    ...
                    if perfil_ativo == 'ADMIN':
                        ...
                        sql_produtos = """
                            SELECT id, codigo_produto, descricao_item, preco_tabela, preco_minimo, preco_index, 
                                   ean_concorrente_1, ean_concorrente_2, ean_concorrente_3, ean_principal, usuario_dono 
                            FROM produtos 
                            WHERE ativo = 1
                            ORDER BY id DESC
                        """
                        ...
                        # No app.py, localize a consulta de listagem de produtos e mude para este formato:
                        sql_produtos = """
                            SELECT id, codigo_produto, descricao_item, preco_tabela, preco_minimo, preco_index, 
                                ean_concorrente_1, ean_concorrente_2, ean_concorrente_3, ean_principal, usuario_dono 
                            FROM produtos 
                    ...
                    # No app.py, localize este trecho dentro da tela == "produtos" e substitua por este:
                    ...
                    for r in rows_produtos:
                        ...
                        # 🔄 CORREÇÃO EXATA DOS ÍNDICES DE PREÇO:
                        ...
                        # 🔄 CORREÇÃO DOS ÍNDICES DOS CONCORRENTES:
                        ...
                        if corresponde:
                            # 🎯 ORDEM COMPATÍVEL COM AS SUAS COLUNAS DO HTML:
                            # [id, codigo, descricao, preco_tabela, preco_minimo, c1, c2, c3, ean_principal, dono, preco_index]
                            produtos_lista.append([
                                id_p, cod_p, desc_p, p_tabela, p_minimo, c1, c2, c3, ean_p, dono_p, preco_index
                            ])
    ...
    return render_template(
        ...
        qtd_itens_cadastrados=qtd_itens_cadastrados  # 📥 Injetado aqui dentro do return
    )
    ...
    try:
        with GerenciadorConexao() as conn:
            with conn.cursor() as cur:
                ...
                # -------------------------------------------------------------
                # 1. Puxa o limite cadastrado para o usuário atual no banco
                ...
                # 2. Se não for um Administrador, verifica a cota de consumo
                if session.get("perfil_logado") != "ADMIN":
                    ...
                    # 3. 🚨 BLOQUEIO INTELIGENTE: Se estourar a cota, envia a mensagem Flash "danger"
                    if (qtd_cadastrada + 1) > limite_permitido:
                        # 🔒 REMOVIDO: A importação 'from flask import flash' que estava quebrando o código
                        flash(f"Ação Negada! Você atingiu o limite operacional máximo de {limite_permitido} requisições permitidas para o seu perfil.", "danger")
                        return redirect(url_for("index", tela="painel"))
                # -------------------------------------------------------------
...
def importar_excel():
    ...
    if file and file.filename.endswith(('.xlsx', '.xls')):
        try:
            ...
            with GerenciadorConexao() as conn:
                with conn.cursor() as cur:
                    # Checagem de limite operacional do usuário
                    cur.execute("SELECT limite_maximo FROM usuario WHERE LOWER(nome) = %s", (usr_dono.lower(),))
                    res_usuario = cur.fetchone()
                    limite_permitido = res_usuario[0] if res_usuario else 100
                    ...
                    for _, row in df.iterrows():
                        ...
                        # 🧠 SUA NOVA REGRA: Captura o valor e limpa. Se for vazio/nan, vira texto em branco
                        valor_flag_bruto = str(row.get('importar_concorrentes', '')).strip().lower()
                        flag_concorrente = "" if valor_flag_bruto == "nan" else valor_flag_bruto
                        ...
                        # 🔍 CONSULTA NO CATÁLOGO BASE DE PRODUTOS
                        cur.execute("""
                            SELECT ean_principal, ean_concorrente_1, ean_concorrente_2, ean_concorrente_3
                            FROM produtos
                            WHERE LOWER(codigo_produto) = %s
                        ...
                        # Se o código do produto não existir no catálogo base, pula a linha
                        if not prod_dados or not prod_dados[0] or str(prod_dados[0]).strip() == "-":
                            itens_ignorados += 1
                            continue
                        ...
                        # 1. Insere o registro Pai na tabela dados
                        query_pai = """
                            INSERT INTO dados (codigo_barras, cidade, proxima_pesquisa, dias, data_cadastro, usuario_dono) 
                            VALUES (%s, %s, %s, %s, CURRENT_TIMESTAMP, %s)
                            RETURNING id
                        """
                        cur.execute(query_pai, (ean_principal, cidade, proxima_pesquisa, dias, usr_dono))
                        novo_id = cur.fetchone()[0]
                        
                        # 2. SEGUNDO A SUA REGRA: Só captura se estiver preenchido estritamente com "sim"
                        concorrentes_lista = []
                        if flag_concorrente == "sim":
                            for idx in [1, 2, 3]:  # Índices para os EANs concorrentes
                                if len(prod_dados) > idx and prod_dados[idx] and str(prod_dados[idx]).strip() != "-":
                                    concorrentes_lista.append(str(prod_dados[idx]).strip())
                                    
                        # 3. Grava no banco seguindo a decisão da flag
                        if concorrentes_lista:
                            cur.executemany(
                                "INSERT INTO dados_concorrentes (dados_id, ean_concorrente) VALUES (%s, %s)",
                            ...
                            # Se na planilha foi marcado "não", deixado em branco, ou se o produto não tinha concorrentes,
                            # grava o bloqueio que mantém a linha limpa na tela principal
    ...
    """Rota secundária acessada via JavaScript para ler o dicionário de progresso."""
    global status_importacao
    return jsonify(status_importacao)
    ...
    try:
        ...
        with GerenciadorConexao() as conn:
            with conn.cursor() as cur:
                ...
                # 3. REGRA RETIFICADA: Se o usuário limpou o campo de texto/desmarcou a flag,
                # inserimos uma marcação explícita de "SEM_CONCORRENTES" para bloquear a busca automática
                if eans_validos:
                    cur.executemany(
                        "INSERT INTO dados_concorrentes (dados_id, ean_concorrente) VALUES (%s, %s)",
                    ...
                    # Salva um registro marcador dizendo que esta tarefa específica NÃO POSSUI concorrentes
        ...
        try:
            with GerenciadorConexao() as conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute("""
                        INSERT INTO produtos (codigo_produto, ean_principal, descricao_item, preco_tabela, preco_minimo, preco_index, ean_concorrente_1, ean_concorrente_2, ean_concorrente_3, usuario_dono) 
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                    """, (cod, ean_p, desc, float(p_tabela_raw or 0), float(p_minimo_raw or 0), float(p_index_raw or 0), e1, e2, e3, usr_dono))
                conn.commit()
...
def baixar_modelo_produtos():
    ...
    try:
        saida = BytesIO()
        # Cria um DataFrame de exemplo estruturado exatamente com as colunas do banco
            ...
            with GerenciadorConexao() as conn:
                with conn.cursor() as cur:
                    for _, row in df.iterrows():
                        ...
                        # Lê buscando pelos novos termos das colunas ou mantendo retrocompatibilidade
                        ...
                        cur.execute("""
                            INSERT INTO produtos (codigo_produto, ean_principal, descricao_item, preco_tabela, preco_minimo, preco_index, ean_concorrente_1, ean_concorrente_2, ean_concorrente_3, usuario_dono) 
                            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                        """, (cod, ean_p, desc, p_tab, p_min, p_idx, e1, e2, e3, usr_dono))
                        sucessos += 1
...
def api_buscar_produto_base(codigo):
    ...
    # 1. Tenta capturar o usuário ativo de qualquer uma das duas chaves possíveis
    ...
    try:
        ...
        with GerenciadorConexao() as conn:
            with conn.cursor() as cur:
                # 2. Executa a busca aplicando LOWER em ambos os lados
                cur.execute("""
                ...
                if prod:
                    ...
                    for idx in range(3, 6):  # Índices dos concorrentes
...
def editar_produto_base():
    ...
    # 🔄 Captura os inputs atualizados vindos dos campos do Modal/Formulário
    ...
    # Se os campos no HTML ainda estiverem com o name antigo por baixo dos panos, o fallback acima protege o código
                ...
                with conn.cursor() as cur:
                    cur.execute("""
                        ...
                        SET codigo_produto = %s, 
                            ...
                            ean_concorrente_3 = %s
                        WHERE id = %s
                    """, (cod, ean_p, desc, float(p_tabela_raw or 0), float(p_minimo_raw or 0), p_index_num, e1, e2, e3, int(id_alvo)))
                conn.commit()
            flash("Produto atualizado com sucesso no catálogo!", "success")
        except Exception as e:
